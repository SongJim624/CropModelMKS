...
module ServerHelper
    ...
    interface
        ...
        integer(4) function  RegCreateKeyEx_Local (hKey ,lpSubKey , &
            ...
            integer(HANDLE)              hKey
            character*(*)                lpSubKey
            integer(DWORD)               Reserved
            integer(HANDLE)              lpClass
            integer(DWORD)               dwOptions
            integer(ULONG)               samDesired
            integer(HANDLE)              lpSecurityAttributes
            integer(HANDLE)              phkResult
            integer(LPDWORD)             lpdwDisposition   
  ...
  contains
    ...
    function RegisterClass(ProgID, Version, ClassName, ThreadingModel, &
        ...
        status1 = RegCreateKeyEx_Local  (HKEY_CLASSES_ROOT, temp_C, 0, &
            ...
            IOR(KEY_WRITE, KEY_QUERY_VALUE), NULL, htopKey, disposition)
        ...
        status1 = RegCreateKeyEx_Local  (htopKey, "CLSID"C, 0, &
            NULL, REG_OPTION_NON_VOLATILE, &
            IOR(KEY_WRITE, KEY_QUERY_VALUE), NULL, h2Key, disposition)
        if (status1 /= ERROR_SUCCESS) goto 99999
    ...
    function RegisterServer () result (r)
        ...
        bDual = .FALSE.
        bDual = .TRUE.
        status = RegisterClass(Component_PROGID, Component_VERSION, &
                    Component_CLASS_NAME, Component_THREADING_MODEL, &
                    CLSID_Component, GUID_TypeLib_CropModelMKS_Fortran_Demonstration, modname, &
                    bDual)
    ...
    function UnregisterServer () result (r)
        ...
        r = S_OK
        !  Per class
        status = UnregisterClass(Component_PROGID, Component_VERSION, &
                    CLSID_Component)
        ...
        !  is installed
        !status = UnRegisterTypeLib(GUID_TypeLib_CropModelMKS_Fortran_Demonstration, ...)
        !  Remove the \TYPELIB sub-keys
        call COMStringFromGUID(GUID_TypeLib_CropModelMKS_Fortran_Demonstration, TypelibGUID, status)
