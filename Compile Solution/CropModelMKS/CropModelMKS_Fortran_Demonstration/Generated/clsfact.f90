...
!  Per class
function IClassFactory_CreateComponentInstance (pData, pUnk, riid, ppv) result (r)
    !dec$ attributes stdcall :: IClassFactory_CreateComponentInstance
    use IClassFactory_Types
    use ifwinty
    use ifcom
    use CropModelMKS_Fortran_Demonstration_global
    use Component_Types
    use IComponent_Methods
    implicit none
    ...
    if ((.not. COMIsEqualGUID(riid, IID_IUnknown)) &
        !  Per interface
         .AND. (.not. COMIsEqualGUID(riid, IID_IComponent)) &
    ...
    allocate (pComponentData)

    !  Per interface
    allocate (pComponentData % IComponent_InternalData % pVtbl)
    pComponentData % IComponent_InternalData % pVtbl % QueryInterface = loc(Component_QueryInterface)
    pComponentData % IComponent_InternalData % pVtbl % AddRef = loc(Component_AddRef)
    pComponentData % IComponent_InternalData % pVtbl % Release = loc(Component_Release)
    pComponentData % IComponent_InternalData % pVtbl % GetTypeInfoCount = loc(Component_GetTypeInfoCount)
    pComponentData % IComponent_InternalData % pVtbl % GetTypeInfo = loc(Component_GetTypeInfo)
    pComponentData % IComponent_InternalData % pVtbl % GetIDsOfNames = loc(Component_GetIDsOfNames)
    pComponentData % IComponent_InternalData % pVtbl % Invoke = loc(Component_Invoke)
    pComponentData % IComponent_InternalData % pVtbl % Update = loc($IComponent_Update)
    pComponentData % IComponent_InternalData % pVtbl % Initialize = loc($IComponent_Initialize)
    pComponentData % IComponent_InternalData % pVtbl % Inquire = loc($IComponent_Inquire)
    pComponentData % IComponent_InternalData % pVtbl % Description = loc($IComponent_Description)
    pComponentData % IComponent_InternalData % pVtbl % Convert = loc($IComponent_Convert)
    pComponentData % IComponent_InternalData % pInternalData => pComponentData % InternalData
    pComponentData % InternalData % pStart => pComponentData
    pComponentData % InternalData % refCount = 1

    !  Allocate the user-defined instance data structure
    allocate (pComponentData % InternalData % pInstanceData)
    !  Call the "constructor"...
    status = Component_CONSTRUCTOR(pComponentData % InternalData % pInstanceData)
    if (status /= S_OK) then
    ...
    if (COMIsEqualGUID(riid, IID_IUnknown)) then
        ppv = loc(pComponentData)
    !  Per interface
    else if (COMIsEqualGUID(riid, IID_IComponent)) then
        ppv = loc(pComponentData % IComponent_InternalData)
